    ...
    def feedback(self, model_name, **kwargs):
        ...
        try:
            if reward is not None and (type(reward) in [int, float]) \
                ...
                _log.info('ignored feedback for model "%s" because invalid arguments: %s' % \
                    (model_name, kwargs))

        except Exception as e:
            _log.warn('Ignore feedback because exception happend. %s' % e)
...
class ProbabilityPolicy(EndpointPolicy):
    ...
    The sum of all model probabilities must be 1.

    Parmeters
    ---------
    model_ratios : dict
        A dictionary specifies the serving ratio for each model for the A/B testng.
        The keys are the model uris, the values are the serving probability of
        each mode. The sum of all serving ratios must be 1.
    ...
    def __init__(self, model_ratios):
        ...
        ratios = model_ratios.values()

        super(ProbabilityPolicy, self).__init__(models, values = None)

        if not ratios:
            ratio = float(1) / len(self._models)
            ratios = [ratio for i in range(len(self._models))]
        else:
            if not len(self._models) == len(ratios):
                raise ValueError('Length of models and ratios should be the same.')
            if not hasattr(ratios, '__iter__') or \
                not all([(r <=1 and r >= 0) for r in ratios]):
                raise ValueError('"ratios" parameter has to be a list of floats between 0 and 1.')
