...
class FeatureHasher(Transformer):
    ...
    * **array.array** : The index of each element is combined with the column name
      and hashed, and the element becomes the value.
    ...
    * **string** : Hash the string and use it as a key, and use 1 as the value.
    ...
    Notes
    -----
    - Each time an entry is hashed, the value is either added or subtracted with
      equal probability. This keeps the value unbiased, since the expectation
      value for each feature(across all examples) is 0.
